...
% NOT verified against the two decomposition papers above -- the gyre
% term uses dx1 (per-gap, per-time, topography-aware width) since it
% needs per-site resolution, while the existing Mov term uses dx2
% (single time-slice, depth-only width), so mean+Mov+gyre may not close
% exactly against a directly-computed total. See the residual check
% near the bottom of this section.
%
% v3: adds a diagnostic block testing whether the residual is mainly
% driven by V0 and wsal using different weighting. wsal is a masked,
% topography-aware weighted mean (weight = dx1, NaN wherever a site is
% below its cutoff depth). V0 divides by the constant, unmasked sum(dx)
% and geo is never NaN-masked at each site's cutoff depth, so V0 and
% wsal are not averaged over the same set of valid gaps at masked
% depths. V0_masked below recomputes V0 the same way wsal is computed,
% and mov_masked/mean_term_masked/gyre_masked/residual_masked let you
% compare against the original (unmasked-V0) residual.
...
% components above. Any residual reflects the dx1-vs-dx2 mismatch
% between gyre and mov, AND the V0-vs-wsal weighting mismatch tested
% below.
...
fprintf('Residual, original V0             (total_direct - [mean+Mov+gyre]): mean=%.4f Sv, max|.|=%.4f Sv\n', ...
        nanmean(residual), nanmax(abs(residual)));

% --------------------------------------------------------
% Diagnostic: V0_masked recomputed with the SAME weighting as wsal --
% mask geo at each site's cutoff depth (same NaN pattern as dx1, coming
% from sal00_mid) and divide by the effective valid width nansum(dx1,3)
% instead of the constant sum(dx). If this shrinks the residual a lot,
% the V0-vs-wsal weighting mismatch was the dominant cause; if not,
% look more closely at the dx1-vs-dx2 (time-invariance) mismatch instead.
geo_masked=geo;
geo_masked(isnan(dx1))=nan;
V0_masked=nansum(geo_masked,3)./nansum(dx1,3);
V00_masked=nanmean(V0_masked);
vel_prime_masked=V0_masked-V00_masked;

aux_masked=vel_prime_masked.*sal_prime*mean(diff(pre));
mov_masked=-nansum(aux_masked.*dx2)/1e6./S0;
mean_term_masked=-V00_masked.*sum(dx2)*mean(diff(pre))/1e6;

vel_pp_masked=vel00-V0_masked;
aux_gyre_masked=vel_pp_masked.*sal_pp.*dx1*mean(diff(pre));
gyre_masked=-nansum(nansum(aux_gyre_masked,3),1)/1e6./S0;

residual_masked=total_direct-(mean_term_masked+mov_masked+gyre_masked);
fprintf('Residual, masked V0 (matches wsal) (total_direct - [mean+Mov+gyre]): mean=%.4f Sv, max|.|=%.4f Sv\n', ...
        nanmean(residual_masked), nanmax(abs(residual_masked)));
...
%save mov_samba_marion dt mov mean_term gyre total_direct mov_masked mean_term_masked gyre_masked
