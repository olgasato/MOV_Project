% MOC "Pilot" method, v3: rebuilt with Kersale et al. (2021)'s ACTUAL
% MOCup definition (see moc_streamfunction_v2.m's header for the full
% derivation from the paper text) -- RAW cumulative transport, NO
% depth-mean/barotropic removal, evaluated at a FIXED depth h_star found
% from the Pilot's OWN time-mean profile (not forced to match the full
% array's h_star -- the paper found each configuration's own transition
% depth independently, and reports they coincide closely: 1315dbar for
% both in their 2013-2017 window).
%
% Keeps v1's BPR-referenced RelTPUD/RefTPUD/Offset derivation (still
% the user's chosen absolute-reference method) and v2's aggregate
% AREA_TOPO_pilot(z) correction (width-weighted average of the 8 real
% per-gap topographic profiles) -- neither of those pieces was in
% question, only the streamfunction/MOC-index definition built on top.
%
% Why this version matters more than v1/v2 did for the reference-method
% question: v1 found BPR and LNM references gave IDENTICAL results,
% because a per-day CONSTANT reference offset cancels exactly under
% depth-mean removal. Without depth-mean removal (this version), a
% constant per-depth-level offset instead accumulates LINEARLY with
% depth through the cumsum -- so the BPR-vs-LNM choice would generally
% matter again here. Not re-tested in this script (BPR remains the
% user's chosen, physically-preferred method), but worth noting for
% anyone tempted to reuse moc_pilot_lnm_v1.m's "reference doesn't
% matter" conclusion -- that conclusion was specific to the abandoned
% depth-mean-removal definition, not a general property of the Pilot
% method itself.
%
% v3 (2026-08-15): first cut with the corrected definition.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Aggregate AREA_TOPO_pilot(z) -- identical construction to v2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% BPR reference correction (same as v1/v2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Relative + reference, same construction as v1/v2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Kersale definition: RAW cumulative transport, no depth-mean
%%% removal, evaluated at h_star found from the Pilot's OWN time-mean
%%% profile.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf('LOWPASS (45-day) Pilot v3 MOCup at h_star: mean=%.4f Sv, std=%.4f Sv\n', nanmean(MOCup_pilot_low), nanstd(MOCup_pilot_low));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Compare against the corrected full-array method (v2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
s_full=load('moc_streamfunction_v2.mat');
...
title('MOC_{up} index at 34.5^oS, Kersale definition: full array vs. Pilot (45-day lowpass)','fontsize',13)
ylabel('MOC_{up} (Sv)')
datetick('x',12);grid;axis('tight')
set(gca,'linewidth',1,'fontsize',14)
print -dpng -r150 moc_pilot_v3_vs_full_IES
...
grid on
print -dpng -r150 moc_pilot_v3_profile_comparison_IES

save('moc_pilot_v3.mat','dt','pre','Psi_raw','Psi_low','h_star_pilot','MOCup_pilot_raw','MOCup_pilot_low','Psi_raw_mean','Psi_std')
