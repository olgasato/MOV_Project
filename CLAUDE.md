...
**Fig 5 reconstruction redone with the fix: `moc_anomaly_fig5_raw_2013_2017_v2.m`.** Same construction as `moc_anomaly_fig5_raw_2013_2017.m` (RAW daily, no lowpass, each series' own 2013-2017 anomaly), just loading `moc_pilot_v6.mat` instead of `v5.mat`. Result: **corr(full anomaly, pilot anomaly) = 0.726** (was 0.485 pre-fix; paper: 0.73) -- full array mean=20.11 Sv/anomaly std=15.39 Sv (paper daily σ: 15.4 Sv), Pilot mean=20.17 Sv/anomaly std=9.65 Sv (paper daily σ: ~9 Sv). Visually (`moc_anomaly_fig5_raw_2013_2017_v2_IES.png`), the two curves now track each other's swings closely throughout the whole 2013-2017 record -- a close match to Kersalé et al. (2021)'s actual Figure 5, unlike the pre-fix version where the Pilot's swings were visibly damped and often out of step with the full array's. User confirmed the match: "ficou perfeito" (2026-08-28).
...
### Correction: the "2018 bump" is real but P4-SPECIFIC, not a shared P1/P2/P4 event -- `moc_p1p2_2018_bump_investigation.m` (2026-08-28)

The previous section's tentative read -- that October 2018 ran a bit higher than July 2017/October 2019 for both P1 and P2 -- was based on only 3-4 sparse CTD snapshots per site, too few points to distinguish a real, time-localized event from ordinary noise. Tested this properly using the FULL, continuous `tau1000_P1`/`tau1000_P2` (and, for comparison, `tau1000_P4`) daily records: fit a smooth seasonal climatology (annual + semi-annual harmonics, least squares, whole record) to each site, then examined the deseasonalized residual's year-by-year mean and standard error (large `n` per year, ~365, giving a properly powered statistical test unlike the single-snapshot CTD comparisons).

**Result -- P1/P2 show NO distinguishable 2018 signal; P4 clearly does**:

| Site | 2016 | 2017 | 2018 | 2019 | 2020 |
|---|---|---|---|---|---|
| P1 (mean±SE, ms) | +0.76±0.07 | +0.93±0.10 | +0.63±0.08 | -0.26±0.11 | -1.30±0.12 |
| P2 (mean±SE, ms) | +0.89±0.10 | +0.86±0.12 | +1.03±0.10 | -1.11±0.15 | -3.01±0.15 |
| **P4** (mean±SE, ms) | +1.45±0.22 | +0.04±0.28 | **+3.36±0.18** | -2.31±0.54 | -2.98±0.26 |

For P1/P2, 2018 is unremarkable relative to neighboring years (2017 is comparably or more elevated at both sites; 2019-2020 actually show a *larger*, opposite-signed excursion -- P2 dips to a 30-day-smoothed minimum of about -8ms in 2020, visually confirmed on `moc_p1p2_2018_bump_zoom_IES.png`, a bigger deviation than anything seen in 2018). For P4, 2018's mean (+3.36ms) is ~19 standard errors from zero and clearly the most extreme year in the record -- a genuine, statistically robust, site-specific feature, not noise.

**Corrects the prior interpretation**: there is no shared, regional "2018 event" spanning P1/P2/P4 -- the 2018 anomaly is real but **localized specifically to P4**, layered on top of the separately-confirmed persistent PIES-vs-CTD baseline offset present at every site, at every time (previous section). This actually simplifies the explanation for the P4-P2 gap's bias: persistent baseline (shared by both sites, roughly cancels in their difference) **plus** a genuine, P4-specific 2018 excursion with no P2 counterpart to cancel it against -- consistent with why it shows up specifically in that gap's difference and not elsewhere.

Outputs: `moc_p1p2_2018_bump_investigation.m`, `moc_p1p2_2018_bump_investigation_IES.png` (full-record residual + 30-day smooth, both sites), `moc_p1p2_2018_bump_zoom_IES.png` (2016-2020 zoom), `moc_p1p2_2018_bump_investigation.mat` (gitignored).
...
- `moc_anomaly_fig5_raw_2013_2017.mat`, `moc_pilot_component_test` (no `.mat` saved), `moc_gpan_noise_test.mat`, `moc_coherence_test.mat`, `moc_pilot_kersale_original_data_test.mat`, `moc_pilot_v6.mat`, `moc_anomaly_fig5_raw_2013_2017_v2.mat`, `moc_gapfill_2017_2020_test.mat`, `moc_p1p2_2018_bump_investigation.mat` — gitignored/not saved. `moc_anomaly_fig5_raw_2013_2017_IES.png`, `moc_pilot_component_test_IES.png`, `moc_gpan_noise_test_IES.png`, `moc_coherence_test_IES.png`, `moc_pilot_v6_vs_full_IES.png`, `moc_pilot_v6_profile_comparison_IES.png`, `moc_anomaly_fig5_raw_2013_2017_v2_IES.png`, `moc_gapfill_2017_2020_test_IES.png`, `moc_p4p2_depth_profile_IES.png`, `moc_p1p2_2018_bump_investigation_IES.png`, `moc_p1p2_2018_bump_zoom_IES.png` — committed (not gitignored). **`moc_pilot_v6.m` is the current, most-correct Pilot script (fixes the BPR-sign and missing-ECCO-reference bugs) — prefer it over `v1`-`v5` going forward.**
