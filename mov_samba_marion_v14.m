...
% --------------------------------------------------------
% v14: flag sustained low-coverage stretches for shading below, instead
% of masking mov to NaN there (v13_9pies tried that -- restricting to
% days where all 9 original sites are simultaneously present leaves a
% ~4-year blank hole, 2017-08/2021-09, and discards the ~54% of days
% that DO have partial coverage. That partial coverage still gives a
% real, if variance-suppressed, computed value -- worth keeping and
% visually flagging rather than deleting). Uses a 30-day rolling mean
% of coverage_totalwidth (smoothing out ordinary day-to-day dropouts,
% which are common throughout the whole record and not what we want to
% flag) and a threshold at 50% of the record's overall median -- not
% sensitive to the exact cutoff, 30-60% all identify essentially the
% same single stretch (~Oct 2017-Oct 2019, matching the P5/P6/P4/P8
% outages documented in CLAUDE.md's "Known limitation" section).
cov_smooth=movmean(coverage_totalwidth,30,'omitnan');
low_coverage=cov_smooth<0.5*median(coverage_totalwidth,'omitnan');
d=diff([0 low_coverage 0]);
lc_starts=find(d==1);
lc_ends=find(d==-1)-1;
fprintf('Low-coverage shading: %d stretch(es), %d of %d days flagged (%.1f%%)\n', ...
    numel(lc_starts), sum(low_coverage), numel(low_coverage), 100*mean(low_coverage));
for kk=1:numel(lc_starts)
    fprintf('  %s to %s\n', datestr(dt(lc_starts(kk))), datestr(dt(lc_ends(kk))));
end
...
for kk=1:numel(lc_starts)
    xpatch=[dt(lc_starts(kk)) dt(lc_ends(kk)) dt(lc_ends(kk)) dt(lc_starts(kk))];
    ypatch=[ylim0(1) ylim0(1) ylim0(2) ylim0(2)];
    fill(xpatch,ypatch,[0.85 0.85 0.85],'EdgeColor','none','FaceAlpha',0.6);
end
...
% coverage_totalwidth by hand. v14 adds low_coverage (the shaded-region
% flag used in the plot above) to the saved output too.
save('mov_samba_marion_v14.mat','dt','mov','mean_term','gyre','total_direct', ...
    'n_gaps_valid','coverage_totalwidth','low_coverage')
