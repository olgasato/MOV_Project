% MOC "Pilot" method, v5: compares against moc_streamfunction_v4.m (the
% calibration-window-fixed full array) instead of v3.m. The Pilot's OWN
% calculation is otherwise UNCHANGED from v4.m -- it never used Step E's
% ECCO time-mean reference step at all (its own reference comes directly
% from BPR, computed independently), so the calibration-window confound
% that motivated moc_streamfunction_v4.m doesn't apply here. The one
% Step E load this script still does (AREA_TOPO_1..8, for the aggregate
% AREA_TOPO_pilot(z)) is also unaffected -- AREA_TOPO is a static,
% time-invariant bathymetric width fraction, untouched by the ECCO
% calibration fix, so it's fine to keep pointing that specific load at
% Step E v2's output (identical to v3's for this variable).
%
% v5 (2026-08-17): only the full-array comparison target changes
% (moc_streamfunction_v3.mat -> v4.mat) and output filenames
% (moc_pilot_v4_* -> moc_pilot_v5_*); every computational step is
% identical to v4.m.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Aggregate AREA_TOPO_pilot(z) -- identical construction to v2/v3/v4,
%%% still pointed at Step E v2's output (fine -- AREA_TOPO is static,
%%% unaffected by the ECCO calibration-window fix, see header note)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% BPR reference correction (same as v1/v2/v3/v4)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Relative + reference, same construction as v1/v2/v3/v4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Shelf transport correction, same static profile the full array
%%% uses -- see moc_streamfunction_v3.m's header for the root-cause
%%% story.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf('LOWPASS (45-day) Pilot v5 MOCup at h_star: mean=%.4f Sv, std=%.4f Sv\n', nanmean(MOCup_pilot_low), nanstd(MOCup_pilot_low));
...
title('MOC_{up} index at 34.5^oS, calib-fixed + shelf: full array vs. Pilot (45-day lowpass)','fontsize',13)
ylabel('MOC_{up} (Sv)')
datetick('x',12);grid;axis('tight')
set(gca,'linewidth',1,'fontsize',14)
print -dpng -r150 moc_pilot_v5_vs_full_IES
...
grid on
print -dpng -r150 moc_pilot_v5_profile_comparison_IES

save('moc_pilot_v5.mat','dt','pre','Psi_raw','Psi_low','h_star_pilot','MOCup_pilot_raw','MOCup_pilot_low','Psi_raw_mean','Psi_std')
