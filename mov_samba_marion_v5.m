...
% NOTE: this changes mov's numeric value (it divides by S0 and centers
% on V00), not just mean_term/gyre -- confirmed with the user before
% applying, since mov traces back to the version used for OSM26.
...
% Mean salinity time series: dx2-weighted depth-mean of wsal (v5), so
% that sal_prime=wsal-S0 integrates to zero when weighted by dx2 across
% depth -- required for mean+Mov+gyre to reconstruct total_direct
% exactly. Previously a double-unweighted mean (unweighted across
% sites via salm, then unweighted across depth), inconsistent with
% wsal's own dx1/dx2 weighting.
...
% V00: dx2-weighted depth-mean of V0 (v5), for the same reason S0 was
% redefined above -- required for vel_prime to integrate to zero when
% weighted by dx2 across depth. Previously nanmean(V0), unweighted.
% vel_prime is the baroclinic (overturning) component.
...
% components above. Should now be close to zero (v5).
